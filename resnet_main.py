...
def resnet_model_fn(features, labels, mode, params):
  ...
  if params['transpose_input'] and mode != tf.estimator.ModeKeys.PREDICT:
    image_size = tf.sqrt(tf.shape(features)[0] / (1 * tf.shape(labels)[0]))
    features = tf.reshape(features, [image_size, image_size, 1, -1])
    features = tf.transpose(features, [3, 0, 1, 2])  # HWCN to NHWC

  # Normalize the image to zero mean and unit variance.
  features -= tf.constant(MEAN_GRAY, shape=[1, 1, 1], dtype=features.dtype)
  features /= tf.constant(STDDEV_GRAY, shape=[1, 1, 1], dtype=features.dtype)
